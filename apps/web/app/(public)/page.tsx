...
export default function LandingPage() {
  ...
  const count1 = useCountUp(50000, statsInView)
      ...
      <div className="min-h-screen bg-[#FAFAFA] text-[#0A0A0A]">
        ...
        <section className="relative mx-auto max-w-6xl px-6 pt-16 pb-20 overflow-hidden">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            {/* Left — text. Uses heroSlideUp (no opacity) so LCP fires on first paint */}
            <div>
              <m.div variants={staggerSlow} initial="hidden" animate="visible">
                ...
                <m.div variants={heroSlideUp} className="flex flex-col sm:flex-row gap-3 mt-8">
                  <Link
                    href="/sign-up"
                ...
                <m.p variants={heroSlideUp} className="text-sm text-gray-400 mt-4">
                  Trusted by 500+ B2B founders across India
                </m.p>
              </m.div>
            </div>

            {/* Right — avatar. Scale-only animation, no opacity: 0 */}
            <m.div
        ...
        <section className="mx-auto max-w-3xl px-6 py-20">
          ...
          >
            {FAQ.map(({ q, a }) => (
              <m.details key={q} variants={fadeUp} className="rounded-xl border border-gray-200 bg-white p-5 group">
